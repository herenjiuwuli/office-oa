...
> ⭐ **这一轮为什么值得加**：它不是「又一个 CRUD 模块」，而是**引入了一类全新的失败模式** —— 在它之前，295 条用例的失败语义几乎全是「单请求 200 或 409」；「一批里有的成、有的不成」是个空白。补它等于给这个 SUT 打开一整面新的缺陷墙 —— 对一个「专门用来被测试的系统」来说，**能失败出新花样**比多一张表值钱得多。
