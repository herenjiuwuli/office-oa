...
| 接口测试 | **vitest** | **295 条**用例（含**跨模块对账**与**批量审批**），见 `tests/` |
| 真机验收 | 自写零依赖 CDP 脚本 | 走真实 Chrome 跑完审批全链路；**失败自动留证**（截图 + 页面文本 + 页面异常），见 `scripts/oa-ui-check.mjs` |
| UI 自动化 | **Playwright**（`channel: 'chrome'`） | **13 条**用例，见 `e2e/`。**不下载浏览器**，详见「UI 自动化」一节 |
| CI | **GitHub Actions** | 静态扫描 → 构建 → 接口测试 → Playwright UI → **真机 CDP 断言（81 条）**，见 `.github/workflows/ci.yml` |
...
| POST | `/api/requests/batch-approve` | 各单据的当前步审批人 | **批量**同意 / 驳回：逐条独立事务；全成功 `200` / 部分成功 `207` / 全失败 `400` |
...
### 13. ⭐ 批量审批：单条审批里根本不存在的那些失败模式（M8）

批量审批的难点从来不是「循环调用审批」，而是**部分成功**：十条里三条已被别人审过、一条已被撤回，返回什么？前端怎么讲清楚？这是单条审批里**根本不会出现**的一类问题。

四个刻意的取舍：

- **① 逐条独立事务，不共用一个「大事务」**。一条失败不该把已经批好的四条一起回滚——用户在批量场景的预期是「能批的先批掉」。所以每条都完整走一次 `actOnRequest`（并发防线、授权顺序、通知副作用全都保持一致），失败只记进该条的结果。
- **② `ids` 必须去重**。`[5,5,5]` 不去重的话，同一张单据的状态机会被**推进三次**——在会签 / 多步流程里足以把它连推两级。这是批量接口最容易漏、后果最严重的边界（用例里专门有一条钉住它）。
- **③ 「不存在」和「跟我无关」对外返回同一种结果**。否则批量接口就成了**单据号枚举器**：挨个试 id，从返回差异就能问出「哪些单据是存在的」。但——**只对真正的「外人」模糊化**：本来就是这张单审批人的人（`canViewRequest` 放行，他本来就能打开它），把原因如实说出来（「你不是当前步骤的审批人」= 单据已被别人推进了）不多泄露任何东西；反而含糊其辞会让「别人先批了」这种正常竞态显得莫名其妙。引擎为此给「外人」错误打了个 `outsider` 标记。
- **④ HTTP 状态码说「这一批整体怎么处理了」，单条原因放 body**：全成功 `200` / **部分成功 `207`** / 全失败 `400`，每条各带 `code`（`conflict` / `not_permitted` / …）与**后端原话**。前端不解析 body 也能知道有没有部分成功。

> ⚠️ **一个只有动手才会撞上的坑**：路径若写成 `/api/requests/batch/approve`（三段），会被上面的 `/api/requests/:id/approve` **按注册顺序先匹配掉**（`:id = "batch"`），然后 `Number("batch") = NaN` 一路走到 SQL 里，变成一个 500。最终用两段的 `/api/requests/batch-approve`，**零冲突**。这和前端 router 里「`/requests/new` 必须放在 `/requests/:id` 之前」是**同一个坑的两种形态**：**带通配段的路径能吞掉任何同形状的固定路径，而且它不报错，只会走进错误的处理器。**

**前端那一半同样重要**：结果面板**逐条列出**（`#7 你不是当前步骤的审批人`），而不是只给一句「成功 1 条、失败 1 条」。批量操作最容易被糊弄过去的地方，就是只报总数、让人自己去列表里找哪条没成。

> 顺带一个接口设计上的两难：全失败返 `400` 时，前端通用错误处理只读 `{error}`，会把 `results` 整个丢掉。所以 400 的 body 里**额外补一句 `error`**，同时批量调用**不复用通用 `request()`** —— 只把「连 `results` 都没有」（参数校验失败）当异常。

> 真机那一层（G7 组）专门验证了这件事：待办列表天然只列「还没人处理的」，**部分成功只会从数据陈旧里冒出来** —— 所以用例是在页面里直接调接口把其中一张先批掉，再回列表批量，断言结果面板真的显示「成功 1 条、失败 1 条 部分成功 207」。
...
>
> ⭐ **④ 层现在失败会自己留证。** 它以前挂了只往 stdout 打几行字，而 CI 里没人盯 stdout ——
> 断言只说得清「没找到元素」，说不清**页面停在哪个 URL、长什么样、控制台报了什么**。
> 现在失败时自动往 `evidence/` 写：
> **截图**（`Page.captureScreenshot`）+ **页面可见文本** + **`Runtime.exceptionThrown` 收下来的页面异常** + 一份 `report.txt`（全量断言结果）。
> CI 里 `if: failure()` 上传成 artifact。
> 造这个能力时验证过两条路：断言失败（`check()` 里触发）、脚本异常中断（`catch` 里强制抓 —— 这一张最值钱，
> 因为之后的分组一条都不会再跑）。留证代码**自己包了 try/catch**：抓现场失败绝不能把原始失败顶掉。
>
> ⚠️ 证据刻意**不放在 `test-results/` 里**（那是 Playwright 的产物目录，它每次启动都会清空它）。
> 这不是洁癖 —— 实测（2026-09-19）：把证据放进去之后，`npm run verify` 里 Playwright 清理该目录时
> 撞上沙箱的批量删除保护（588 个文件 > 阈值 50），**一条 E2E 都没跑就死了**。
> 「证据被别人的清理顺手删掉」和「证据害得别人的清理失败」，两个都不想要 —— 所以另起目录。

### 接口测试（vitest）

- **295 条用例，14 个文件**：`auth` / `permission` / `flow` / `requests` / `ai` / `attachments` / `attachments-edge`（附件的越权 / 并发 / 边界）/ `notifications`（M3 站内通知）/ `export`（M4 导出 CSV）/ `meetings`（M5 会议室）/ `stats`（M6 统计报表）/ `attendance`（M7 考勤：打卡幂等 / 本人记录 / scope 越权 403 / 数字自洽）/ **`consistency`（跨模块对账，见下）** / **`batch-approve`（批量审批：部分成功语义 / id 去重 / 存在性不泄露）**
...
### ⭐ 跨模块对账（`tests/consistency.test.js`，12 条）

每个模块内部都有自己的用例：导出测了转义与公式注入、统计测了 scope 与越界 403、通知测了收件人隔离、
列表测了可见性 —— 它们**全绿**。这个文件只问一件事：**模块之间对得上吗？**

为什么必须单独一层：**口径不一致是沉默的。**

| 不一致的表现 | 为什么没人发现 |
|---|---|
| 列表说 18 条、导出悄悄给你 20 条 | 两个页面都「正常」，没有断言会红 —— **而且这不是显示问题，它就是越权** |
| 统计说「全公司 37 条」、列表说 36 条 | 没人会去对着看两个页面的数字 |
| 提交成功了，审批人却没收到通知 | 主操作返回 200，副作用静默丢掉 |

M4 把可见性抽成 `scopeFilter()` 让列表与导出共用，正是为了防第一行那种事 —— 但
「抽出来了」只是**代码约定**：在写下这个文件之前，**没有任何一条用例证明它真的被共用了**，
全靠人读代码相信。这个文件把「相信」换成「证明」，12 条用例覆盖四组关系：

- **A 列表 ↔ 导出**：同一筛选下「CSV 数据行数 == 列表条数 == `X-Total-Count`」；筛选真的起作用（比不筛时少）；`?mine=1` 一起缩回。
  ⭐ 顺带记一个**数数的坑**：标题里带换行时，用「按 `\r\n` 切行」的朴素写法会**多算行数** → 这条对账断言会**因为数错而假红**。
  所以这里自己写了 RFC4180 的记录切分（**引号内的 CRLF 不算换行**），并用「朴素数法必须多算」当反证钉住它。
- **B 列表 ↔ 统计**：同一范围下 `stats.requests.total == 列表条数`。
  ⭐ 并**钉住一处刻意的不对称**：部门经理的统计看得见「本部门」，单据明细却只看得到自己（`dept_manager` 没有 `request:read:all`）。
  这是「**聚合可见 / 明细不可见**」的合理设计，但以前没有任何用例记录它 —— 哪天有人「顺手修好」，
  注意 `request:read:all` **只有「全部」一档、没有「仅本部门」这一档**：补上去拿到的是**全公司**明细，爆炸半径比想象中大。
- **C 通知 ↔ 业务事件**：一次提交 = 审批人恰好 +1 条；一次归档 = 申请人恰好 +1 条；
  撤回 = 待审的人各 +1 条 `cancelled` 且撤回的人不给自己发。**不多不少** —— 防的正是「主操作成功、副作用静默丢失」。
- **D 端到端对账**：一条链路走完（建单→提交→一级→二级→归档），**列表 / 导出 / 统计必须同时 +1**，
  两个该收通知的人也**各 +1**。任何一处没动，都说明它算的是**别的东西**。

> 条数一律**动态取基准**（先读一次列表再比），绝不写死种子条数 —— seed 一改，写死的数字就会假失败。

### 真机浏览器验收（UI 层）

```bash
npm start                        # 或 npm run dev（dev 时改传 http://127.0.0.1:5273）
node scripts/oa-ui-check.mjs     # 81 条断言，走一段就全过
```
...
| **G7 批量审批（M8）** | 待办每行有勾选框且点得动；勾选后出现批量栏并写明「已选 N 条」；⭐ **部分成功在界面上说得清** —— 先在页面里调接口抢批一张（构造数据陈旧），再回列表批量，结果面板如实显示「成功 1 条、失败 1 条 · 部分成功 207」；失败那条给的是**后端原话**（「你不是当前步骤的审批人」）而不是前端自己编的「操作失败」；处理完两张都离开待办 |
...
这两条都不是「代码报错」，是 **81 条**业务断言逼出来的。光靠 `npm test` + `npm run check:frontend` 一个都发现不了。
...
| 断言/重试/报告 | 自己写（81 条手写断言） | 框架自带（自动等待、重试、trace、HTML 报告） |
...
| **「CSV 按行数」会数错** | 断言「导出记录数 == 列表条数」时用 `split('\r\n')` 数行：标题里带 CRLF 的字段会被当成记录分隔符 → **数多** → 这条对账断言**因为数错而假红**（不是产品坏了，是你数错了） | 数 CSV 记录必须用 RFC4180 的切分：**引号内的 CRLF 不算换行**。`tests/consistency.test.js` 自带 `csvRecordCount()`，并反证「朴素数法必须多算」。⚠️ 写这条反证时数据要用 **CRLF**（`\r\n`）—— 第一版写成裸 `\n`，字段里没有 CRLF、朴素数法不会多数，**反证直接假红**（已修） |
| **「抓现场」的代码自己崩了会盖住原始失败** | 给真机断言加失败留证（截图 / 页面文本）时，如果留证代码抛错，会把「产品为什么挂」的真答案顶掉，反而更难查 | 留证逻辑**整体包 try/catch，且永不抛错**；写入用同步 `writeFileSync`；排队的留证任务在关浏览器**之前** `Promise.allSettled` flush 掉（连接一关就截不了图）。断言失败与脚本异常两条路都验过 |
...
1. ✅ **批量审批** —— `batchActOnRequest()`（**逐条复用 `actOnRequest`**，每条一个独立事务，因此并发防线 / 授权顺序 / 通知副作用全部沿用）+ 路由 `POST /api/requests/batch-approve` + 前端 `Todo.vue`（多选 / 全选 / 批量操作栏 / **逐条结果面板**）+ **24 条接口用例 + 8 条真机断言**（G7 组）。见「关键设计决策」第 13 条
2. ⭐ **把「部分成功」当成一类正经的失败模式**：全成功 `200` / 部分成功 `207` / 全失败 `400`；每条各带 `code` 与**后端原话**，前端不解析 body 也知道有没有部分成功
3. ⭐ **`ids` 去重**：`[5,5,5]` 只推进一格（不去重会把同一张单据的状态机连推三次）
4. ⭐ **存在性不泄露，但只对「外人」**：引擎给「外人」错误打 `outsider` 标记 → 批量接口对「不存在」和「与我无关」返回同一种结果（防 id 枚举）；而单据真正的审批人仍能拿到具体原因 —— 否则「**别人先批了**」会显示成「无权处理该单据」，让人莫名其妙
5. ⚠️ **路径写成 `/batch/approve` 会被吞掉**：三段路径会被 `/api/requests/:id/approve` 按注册顺序先匹配（`:id = "batch"` → `Number("batch") = NaN` → 一路走到 SQL 变成 500）。最终用两段的 `/batch-approve`，零冲突
6. ⚠️ **全失败返 400 会让前端丢掉 `results`**（通用错误处理只读 `{error}`）→ 400 的 body 里额外补一句 `error`，且批量调用**不复用通用 `request()`**

> ⭐ **这一轮为什么值得加**：它不是「又一个 CRUD 模块」，而是**引入了一类全新的失败模式** —— 在它之前，271 条用例的失败语义几乎全是「单请求 200 或 409」；「一批里有的成、有的不成」是个空白。补它等于给这个 SUT 打开一整面新的缺陷墙 —— 对一个「专门用来被测试的系统」来说，**能失败出新花样**比多一张表值钱得多。
...
| 文件 | 内容 |
|---|---|
| [`docs/面试弹药-office-oa.md`](docs/面试弹药-office-oa.md) | 一句话定位 / **15 个**技术亮点 / 「我改过的点 + 为什么」**36 条**候选清单 / **38 道**高频深挖题 + 答案 / 一句话收尾（含 AI、并发、安全、验收四个备用收尾） |
